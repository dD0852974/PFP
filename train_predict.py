...
def load_pretrained_weights(ckpt_path): 
    adjusted_weights = {};
    pretrained_weights = torch.load(ckpt_path, map_location='cpu'); #用来加载torch.save() 保存的模型文件。
...
def parse_args():
    parser = argparse.ArgumentParser(description='Video Clip Restruction and Playback Rate Prediction')  #https://dboyliao.medium.com/python-%E8%B6%85%E5%A5%BD%E7%94%A8%E6%A8%99%E6%BA%96%E5%87%BD%E5%BC%8F%E5%BA%AB-argparse-4eab2e9dcc69
...
def main():
    args = parse_args()
    print(vars(args))
    os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu #https://blog.gtwang.org/programming/tensorflow-keras-specify-gpu-and-memory-tutorial/ 使用特定的 GPU 卡，可以使用 CUDA_VISIBLE_DEVICES 這個環境變數來設定，它的值代表 CUDA 程式可以使用的 GPU 卡編號（從 0 開始）
    ...
    train_loader = DataLoader(train_dataset,batch_size=params['batch_size'],shuffle=True,num_workers=params['num_workers'],drop_last=True) #https://blog.csdn.net/g11d111/article/details/81504637
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', min_lr=1e-7, patience=50, factor=0.1) #当网络的评价指标不在提升的时候，可以通过降低网络的学习率来提高网络性能https://blog.csdn.net/weixin_40100431/article/details/84311430
    save_path = params['save_path_base'] + "train_predict_{}_".format(args.exp_name) + params['data']
    model_save_dir = os.path.join(save_path,time.strftime('%m-%d-%H-%M')) #用于格式化时间，返回以可读字符串表示的当地时间
